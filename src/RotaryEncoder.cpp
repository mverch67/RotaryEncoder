/* Rotary encoder handler for arduino/platformio
 *
 ...
 */
  ...
  * Changed state table to follow sequence 00>10>11>01>00
  * Added methods to read button state, check for pressed and released, and pressed and held
  * Added method to reset button state
  */

 /*
  * Modified 8/24/2025 by mverch67
  * Changed class name from Rotary to RotaryEncoder to avoid conflicts
  * Changed file names to RotaryEncoder.*h/.cpp
  * Changed #defines to enum class
  * Added library.properties and library.json for arduino/platformio compatibility
  */
 ...
 */
RotaryEncoder::RotaryEncoder(uint8_t _pin1, uint8_t _pin2) {
 ...
 */
void RotaryEncoder::init(uint8_t _pin1, uint8_t _pin2) {
  // Assign variables.
  pin1 = _pin1;
  pin2 = _pin2;
  // Set pins to input.
#ifdef ENABLE_ROTARY_PULLUP
...
*/
bool RotaryEncoder::buttonPressedReleased(uint16_t debounce_delay) {
  ...
  if (buttonState == (BUTTON_PRESSED | BUTTON_RELEASED)) { 
    buttonState = BUTTON_RESET;                         // Reset the button state before returning true
    return true;
  }
  else {
    return false;
...
*/
bool RotaryEncoder::buttonPressedHeld(uint16_t delay_millis) {
